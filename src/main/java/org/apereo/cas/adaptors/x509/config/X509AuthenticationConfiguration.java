...
import br.com.lucas.cas.adaptors.x509.authentication.principal.X509SubjectICPBrasilPrincipalResolver;
...
public class X509AuthenticationConfiguration {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(X509AuthenticationConfiguration.class);
    ...
    @Bean
    @RefreshScope
    public PrincipalResolver x509SubjectICPBrasilPrincipalResolver() {
        final X509Properties x509 = casProperties.getAuthn().getX509();

        final X509SubjectICPBrasilPrincipalResolver r = new X509SubjectICPBrasilPrincipalResolver(attributeRepository,
                x509PrincipalFactory(),
                x509.getPrincipal().isReturnNull(),
                x509.getPrincipal().getPrincipalAttribute());

        if(x509.getLdap().getLdapUrl() != null && x509.getLdap().getBaseDn() != null) {
            LOGGER.debug("LDAP URL and BaseDN informed for x509 dn resolution");
            final PooledSearchDnResolver resolver = new PooledSearchDnResolver();
            resolver.setBaseDn(x509.getLdap().getBaseDn());
            resolver.setSubtreeSearch(x509.getLdap().isSubtreeSearch());
            resolver.setAllowMultipleDns(x509.getLdap().isAllowMultipleDns());
            resolver.setConnectionFactory(LdapUtils.newLdaptivePooledConnectionFactory(x509.getLdap()));
            resolver.setUserFilter(x509.getLdap().getSearchFilter());
            r.setDnResolver(resolver);
        }
            ...
            if (casProperties.getAuthn().getX509().getPrincipalType() != null) {
                switch (casProperties.getAuthn().getX509().getPrincipalType()) {
                    ...
                    case CN_EDIPI:
                        ...
                        break;
                    case ICPBRASIL:
                        resolver = x509SubjectICPBrasilPrincipalResolver();
                        break;
                    default:
