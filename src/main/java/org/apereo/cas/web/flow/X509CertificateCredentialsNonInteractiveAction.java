...
public class X509CertificateCredentialsNonInteractiveAction extends AbstractNonInteractiveCredentialsAction {
    ...
    protected Credential constructCredentialsFromRequest(final RequestContext context) {
        ...
        final HttpServletRequest nativeReq = (HttpServletRequest) context.getExternalContext().getNativeRequest();

        if (certificates == null || certificates.length == 0 || ! nativeReq.getRequestURI().toLowerCase().contains("x509")) {
